...
export const FileUploadSection = ({ onFileChange }: FileUploadSectionProps) => {
  const handleFileSelect = useCallback((files: File[]) => {
    if (files.length > 0) {
      ...
      const isPDFType = file.type === 'application/pdf' || fileExtension === 'pdf';
      const isRTFType = file.type === 'text/rtf' || fileExtension === 'rtf';
      const isCSVType = file.type === 'text/csv' || fileExtension === 'csv';
      const isExcelType = file.type.includes('spreadsheet') || fileExtension === 'xlsx' || fileExtension === 'xls';
      
      if (!isPDFType && !isRTFType && !isCSVType && !isExcelType) {
        alert('Παρακαλώ επιλέξτε αρχείο PDF, RTF, CSV ή Excel');
        return;
      }
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground">
                  Σύρετε και αποθέστε ένα PDF, RTF, CSV ή Excel αρχείο ή κάντε κλικ για επιλογή
                </p>
              </div>
